...
reload_model_transformer = tf.keras.models.load_model('./transformer_model.h5')
...
# =================================================================================================
    ...
    if selected_status_7 == 7:
        spots_train = spots_train[282:]
        x_test = x_test[:7]
        y_test = y_test[:7]
    elif selected_status_7 == 14:
        spots_train = spots_train[275:]
        x_test = x_test[:14]
        y_test = y_test[:14]
    elif selected_status_7 == 21:
        spots_train = spots_train[268:]
        x_test = x_test[:21]
        y_test = y_test[:21]
    elif selected_status_7 == 28:
        spots_train = spots_train[261:]
        x_test = x_test[:28]
        y_test = y_test[:28]
    ...
    def plot_multi_step(history, prediction1):
        ...
        range_future = list(range(range_history, range_history + len(prediction1)))
        plt.plot(np.arange(range_history), np.array(history), label='History',linewidth=3)
        plt.plot(range_future, np.array(prediction1),label='Forecasted with Transformer',linewidth=3)
        plt.legend(loc='upper right',fontsize=20)
        plt.xlabel('Time step(days)',fontsize=24)
        plt.ylabel('EV charging demand(Total_kwh)',fontsize=24)
        plt.xticks(fontsize=20)
